...
- ⠇ Меню по наведению на иконку «три точки» у модуля/урока: 
  - Дублировать модуль/урок (перед выполнением открывается форма редактирования с префиксом «КОПИЯ»)
  - Удалить модуль/урок (с подтверждением)
...
Во время долгих операций отображаются индикаторы загрузки (спиннеры/тосты). Кнопки, не имеющие данных (например, «Просмотр детального контента» при его отсутствии), блокируются.
...
- Если детального контента нет — кнопка просмотра/экспорта заблокирована, подсказка предлагает сначала сгенерировать контент

---

### 4.1 🎥 Генерация и просмотр видео урока (опционально)
```
Текст урока → Отправка в HeyGen → Ожидание статуса → Просмотр/скачивание
```

**Что делает пользователь:**
- Открывает форму «Генерация видео» у урока
- Выбирает голос и аватар (подтягиваются из API; при загрузке показываются индикаторы)
- Запускает генерацию видео и ожидает готовности

**Что делает система:**
- Сохраняет кэш запроса на генерацию, повторные запросы могут брать результат из кэша
- Опрос статуса у HeyGen до «completed» с интервалом
- Сохраняет `video_id` и `download_url` в БД/кэше

**Просмотр:**
- При нажатии «Смотреть видео» показывается тост/индикатор загрузки, затем открывается ссылка на видео
