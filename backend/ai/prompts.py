...
- Каждый слайд — развёрнутый материал: контекст (зачем это, связь с темой), объяснения, определения.
- Обязательно включай развёрнутые примеры: не один тезис, а «как правильно / как неправильно» или «до — после» с пояснениями.
- НЕ пиши краткие тезисы! Пиши полноценный учебный текст с контекстом и примерами.
...
- В "content" — развёрнутый материал: контекст (зачем этот слайд, связь с темой), 2-4 абзаца или пункты с пояснениями.
- Примеры давай развёрнуто: не только «хороший вариант», но и «плохой вариант» или «до/после» с кратким пояснением почему.
- Для code слайдов — полноценные примеры кода с комментариями в code_example (смысл строк, типичные ошибки).
- В "notes" — краткие методические указания для преподавателя (как подать материал, на что обратить внимание).
- Нумеруй слайды с 1. content — основной учебный материал для студента (не тезисы, а текст с контекстом и примерами).
...
СТРУКТУРА КОНТЕНТА НА СЛАЙДАХ (где уместно используй этот каркас):
- Контекст и роль: зачем это нужно, как связано с темой и с предыдущим материалом (2-3 предложения).
- Задача и инструкции: что именно нужно сделать или понять, пошагово.
- Формат вывода: как должен выглядеть результат (для промптов — пример фразы; для кода — структура ответа).
- Примеры и ограничения: 1-2 развёрнутых примера (хороший/плохой вариант, до/после, типичные ошибки).
- Тон и стиль: как формулировать (официально/дружелюбно, кратко/подробно) — одним абзацем при необходимости.

ТРЕБОВАНИЯ К СЛАЙДАМ:
- Каждый слайд — развёрнутый материал: минимум 3-4 абзаца по 2-5 предложений или пункты с пояснениями после каждого.
- Обязательно добавляй КОНТЕКСТ: зачем этот блок, как он связан с целями урока.
- Примеры: не один тезис, а развёрнуто (например: «Плохо: … Почему плохо. Хорошо: … Почему лучше.»).
- Для технических тем — полноценные примеры кода в code_example с комментариями (минимум 2 слайда с кодом).
- learning_objectives (3-4 цели) и key_takeaways (3-4 вывода) — конкретные и измеримые формулировки.
...
{{
  ...
  "slides": [
    {{
      ...
      "content": "Контекст: зачем этот блок и как он связан с темой. Основной материал с объяснениями (2-4 абзаца). Примеры: развёрнуто, с пояснением «почему так» и «как не надо» где уместно.",
      "slide_type": "content",
      "code_example": null,
